...
class Agent:
    ...
    def generate_expression(self, query):
        response = self.ppo_trainer.generate(query.to(self.device), **self.generation_kwargs)
        return response
        ...
        for _, line in data.iterrows():    
            ...
            while self.stop_condition():
                ...
                for query in tqdm(query_tensors):
                    ...
                    response = self.generate_expression(query)
                    while self.check_expression(response):
                        response = self.generate_expression(query)

                    response = response.squeeze()[-self.generation_kwargs["max_new_tokens"]:]
